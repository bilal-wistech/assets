...
class Helper
{
    ...
    public static function checkUninsuredUsers($asset_id)
    {
        $asset = DB::table('assets')->where('id', $asset_id)->first();
        // return $asset;
        if ($asset->uninsured_users == 1) {
            // dd('hi broh i am uninsured, i have access to all');
            return true;
        } else {
            // dd('by broh i have access to only a few');
            return false;
        }
    }
    ...
    public static function convertDateTimeUser($datetimeStr)
    {
    ...
    */
    ...
    public static function convertDateTimeFormat($datetimeStr)
    {
        $datetime = Carbon::createFromFormat('Y-m-d H:i:s', $datetimeStr);
        $formattedDatetime = $datetime->format('Y-M-d');
        $time = $datetime->format('h:i A');
        $html = '<div class="btn btn-color" style="background-color: yellow ; padding:0px">';
        $html .= '<span style="color: blue;">' . $time . '</span>';
        $html .= '</div>';
        return $formattedDatetime . ' ' . $html;
    }
    ...
    {
        $assets = Asset::select('id', DB::raw("CONCAT(name, ' (', asset_tag, ')') as formatted_name"))->pluck('formatted_name', 'id')->toArray();
        return $assets;
    }
    public static function getReadyAssets()
    {
        $assets = Asset::whereNull('assigned_to')
            ->join('status_labels AS status_alias', function ($join) {
                $join->on('status_alias.id', '=', 'status_id')
                    ->where('status_alias.deployable', '=', 1)
                    ->where('status_alias.pending', '=', 0)
                    ->where('status_alias.archived', '=', 0);
            })
            ->select('assets.id', DB::raw("CONCAT(assets.name, ' (', assets.asset_tag, ')') as formatted_name"))
            ->pluck('formatted_name', 'assets.id')
            ->toArray();

        return $assets;
    }

    public static function getDateFormate($datetime = null)
    {
        return Carbon::parse($datetime)->toDateString();
    }

    public static function getNumberFormate($value = null)
    {
        return number_format($value);
    }


    public static function getPremiumTypeLabel()
    {
        return [
            'per_month' => '<label class="label label-default label-secondary">Per Month</label>',
            'per_year' => '<label class="label label-default label-success">Per Year</label>',
        ];
    }


    public static function getDriverNamesArr()
    {
        $group_id = [2, 3];
        $users = User::select(['id', DB::raw('CONCAT(first_name, " ", last_name) AS show_name')])
    ...
    public static function getUsersNames($allowedDriverIds)
    {
        $users = User::select(['id', DB::raw('CONCAT(first_name, " ", last_name) AS show_name')])
    ...
    public static function getSuppliersArr()
    {
        return Supplier::select('id', 'name')->pluck('name', 'id')->toArray();
    }

    public static function showMessage($received)
    {
        if ($received) {
            return '<span style="background-color: green; color: white; padding: 5px; border-radius: 5px;">&#9873; Received</span>';
        } else {
            return '<span style="background-color: red; color: white; padding: 5px; border-radius: 5px;">&#9872; Not Received</span>';
        }
    }
}
