...
class AssetsTransformer
{
    ...
    public function transformAsset(Asset $asset)
    {
        ...
        $array = [
            ...
            // 'assigned_to' => $this->customAssignedTo($asset),
        ...
        $array['custom_checkincheckout'] = $this->customCheckinCheckout($asset);
    ...
    public function customCheckinCheckout($asset){
        if($asset->last_checkout==null){
            return '<a href="'.url("hardware/".$asset->id."/createCheckout").'" class="btn btn-sm bg-blue" data-tooltip="true" title="">Checkout</a>';
        }

        return '<a href="'.url("hardware/".$asset->id."/checkin").'" class="btn btn-sm bg-purple" data-tooltip="true" title="">Checkin</a>';
        
    }

    public function customAssignedTo($asset)
    {
        $checkout_to='';
        if(isset($asset->getCheckOutInfo->getCheckOutUsersInfo) && $asset->getCheckOutInfo->getCheckOutUsersInfo<>null){
            foreach($asset->getCheckOutInfo->getCheckOutUsersInfo as $key => $checkout){
               $checkout_to .= '<a href="'.url('users/'.$checkout->getUsersInfo->id).'" class="label label-primary" style="margin-right: 3px;">'.$checkout->getUsersInfo->show_name.'</a>';
            }
        }
        return $checkout_to;
    }
