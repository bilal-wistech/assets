...
Route::group(['prefix' => 'v1', 'middleware' => ['api', 'throttle:api']], function () {
    Route::group(['prefix' => 'repairoptions'], function () {

        Route::get(
            '/repair-options',
            [
                Api\RepairOptionsController::class,
                'index'
            ]
        );
    });
...
// handover details
        ...
        )->name('api.expenses.handover-details');
    });
...
Route::post('/asset_maintenance_type_create', [
    Api\AssetMaintenancesController::class, 'createType'
])->name('api.asset_maintenance_type_create')->middleware('auth:api');
...
Route::group(['prefix' => 'v1', 'middleware' => ['api', 'throttle:api']], function () {
    ...
    Route::resource(
        'daily-earning',
        Api\DailyEarningReportController::class,
        [
            'names' => [
                'index' => 'api.daily-earning.index',
            ],
            'except' => ['create', 'store', 'edit', 'update', 'destroy'],
            'parameters' => ['daily-earning' => 'daily_earning_id'],
        ]
    );
    ...
    ); // end insurance API routes


    Route::get('upcomming-expired-insurances', [Api\InsuranceController::class, 'upcommingExpired'])
        ->name('api.upcomming.expired.insurances');


    Route::resource(
        'asset-assignment',
        Api\AssetAssignmentController::class,
        [
            'names' => [
                'index' => 'api.assetassignment.index',
                'show' => 'api.insurance.show',
                'update' => 'api.insurance.update',
                'store' => 'api.insurance.store',
                'destroy' => 'api.insurance.destroy',
