    ...
    <script nonce="{{ csrf_token() }}">
        ...
        function genericColumnObjLinkFormatter(destination) {
            return function(value, row) {
                if ((value) && (value.status_meta)) {
                    ...
                    return '<nobr><a href="{{ url('/') }}/' + destination + '/' + value.id +
                        '" data-toggle="tooltip" title="' + status_meta[value.status_meta] + '"> <i class="fa ' +
                    ...
                    return '<nobr><a href="{{ url('/') }}/' + dpolymorphicItemFormatterest + dest + '/' + value
                        .id + '">' + value.name + '</a></span>';
                }
            };
        }

        function hardwareAuditFormatter(value, row) {
            return '<a href="{{ url('/') }}/hardware/audit/' + row.id +
            ...
            return function(value, row) {
                ...
                if ((row.available_actions) && (row.available_actions.clone === true)) {
                    actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id +
                        '/clone" class="btn btn-sm btn-info" data-tooltip="true" title="{{ trans('general.clone_item') }}"><i class="far fa-clone" aria-hidden="true"></i><span class="sr-only">Clone</span></a>&nbsp;';
                }

                if ((row.available_actions) && (row.available_actions.update === true)) {
                    actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id +
                        '/edit" class="btn btn-sm btn-warning" data-tooltip="true" title="{{ trans('general.update') }}"><i class="fas fa-pencil-alt" aria-hidden="true"></i><span class="sr-only">{{ trans('general.update') }}</span></a>&nbsp;';
                }
                ...
                if ((row.available_actions) && (row.available_actions.delete === true)) {
                    ...
                    if (dest == 'insurance') {
                        ...
                        actions += '<a href="https://asset.wistech.biz/' + dest + '/' + row.id + '/delete" ' +
                            ...
                            ' data-content="{{ trans('general.sure_to_delete') }} ' + name_for_box + '?" ' +
                            ' data-title="{{ trans('general.delete') }}" onClick="return false;">' +
                            '<i class="fa-solid fa-user-slash"></i><span class="sr-only">{{ trans('general.delete') }}</span></a>&nbsp;';

                    } else if (dest == 'expensetype') {
                        name_for_box = 'expensetype';
                        actions += '<a href="https://asset.wistech.biz/' + dest + '/' + row.id + '/delete" ' +
                            ' class="actions btn btn-danger btn-sm delete-asset" data-tooltip="true"  ' +
                            ' data-toggle="modal" ' +
                            ' data-content="{{ trans('general.sure_to_delete') }} ' + name_for_box + '?" ' +
                    ...
                    } else if (dest == 'tsrepairoptions') {
                        name_for_box = 'tsrepairoptions';
                        actions += '<a href="https://asset.wistech.biz/' + dest + '/' + row.id + '/delete" ' +
                            ' class="actions btn btn-danger btn-sm delete-asset" data-tooltip="true"  ' +
                            ' data-toggle="modal" ' +
                            ' data-content="{{ trans('general.sure_to_delete') }} ' + name_for_box + '?" ' +
                    ...
                    } else if (dest == 'assetassignment') {
                        name_for_box = 'assetassignment';
                        actions += '<a href="{{ config('app.url') }}/' + dest + '/' + row.id + '/delete" ' +
                            ' class="actions btn btn-danger btn-sm delete-asset" data-tooltip="true"  ' +
                            ' data-toggle="modal" ' +
                            ' data-content="{{ trans('general.sure_to_delete') }} ' + name_for_box + '?" ' +
                    ...
                    actions +=
                        '<a class="btn btn-danger btn-sm delete-asset disabled" onClick="return false;"><i class="fas fa-trash"></i></a>&nbsp;';
                }


                if ((row.available_actions) && (row.available_actions.restore === true)) {
                    actions += '<form style="display: inline;" method="POST" action="{{ url('/') }}/' + dest +
                        '/' + row.id + '/restore"> ';
                    actions += '@csrf';
                    actions +=
                        '<button class="btn btn-sm btn-warning" data-toggle="tooltip" title="{{ trans('general.restore') }}"><i class="fas fa-retweet"></i></button>&nbsp;';
                }
        ...
        function polymorphicItemFormatter(value) {
            ...
            if ((value) && (value.type)) {
                ...
                return '<nobr><a href="{{ url('/') }}/' + item_destination + '/' + value.id +
        ...
        function licenseSeatInOutFormatter(value, row) {
            ...
            if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && ((!row.asset_id) && (!row
                    .assigned_to))) {
                return '<a href="{{ url('/') }}/licenses/' + row.license_id + '/checkout/' + row.id +
                    '" class="btn btn-sm bg-maroon" data-toggle="tooltip" title="{{ trans('general.checkout_tooltip') }}">{{ trans('general.checkout') }}</a>';
            } else {
                return '<a href="{{ url('/') }}/licenses/' + row.id +
                    '/checkin" class="btn btn-sm bg-purple" data-toggle="tooltip" title="Check in this license seat.">{{ trans('general.checkin') }}</a>';
            }
        ...
        function genericCheckinCheckoutFormatter(destination) {
            return function(value, row) {
                ...
                if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && ((!row.asset_id) && (
                        !row.assigned_to))) {
                    return '<a href="{{ url('/') }}/' + destination + '/' + row.id +
                        '/checkout" class="btn btn-sm bg-maroon" data-toggle="tooltip" title="{{ trans('general.checkout_tooltip') }}">{{ trans('general.checkout') }}</a>';
                    ...
                    if (row.assigned_to) {
                        return '<a href="{{ url('/') }}/' + destination + '/' + row.id +
                            '/checkin" class="btn btn-sm bg-purple" data-toggle="tooltip" title="Check this item in so it is available for re-imaging, re-issue, etc.">{{ trans('general.checkin') }}</a>';
                    } else if (row.assigned_pivot_id) {
                        return '<a href="{{ url('/') }}/' + destination + '/' + row.assigned_pivot_id +
                            '/checkin" class="btn btn-sm bg-purple" data-toggle="tooltip" title="Check this item in so it is available for re-imaging, re-issue, etc.">{{ trans('general.checkin') }}</a>';
                    }
        ...
        function assetRequestActionsFormatter(row, value) {
            if (value.assigned_to_self == true) {
                ...
                return '<form action="{{ url('/') }}/account/request-asset/' + value.id +
                    '" method="POST">@csrf<button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Cancel this item request">{{ trans('button.cancel') }}</button></form>';
            } else if (value.available_actions.request == true) {
                return '<form action="{{ url('/') }}/account/request-asset/' + value.id +
                    '" method="POST">@csrf<button class="btn btn-primary btn-sm" data-toggle="tooltip" title="Request this item">{{ trans('button.request') }}</button></form>';
            }
            ...
            if ((row.custom_fields) && (row.custom_fields[field_column_plain])) {
                ...
                if ((row.custom_fields[field_column_plain].field_format) && (row.custom_fields[field_column_plain].value)) {
                    if (row.custom_fields[field_column_plain].field_format == 'URL') {
                        ...
                        return '<a href="mailto:' + row.custom_fields[field_column_plain].value + '">' + row.custom_fields[
                            field_column_plain].value + '</a>';
                    }
                }
        ...
        function groupsFormatter(value) {
            ...
            if (value) {
                ...
                for (var index in value.rows) {
                    groups += '<a href="{{ url('/') }}/admin/groups/' + value.rows[index].id +
        ...
        function changeLogFormatter(value) {
            ...
            for (var index in value) {
                result += index + ': <del>' + value[index].old +
                    '</del>  <i class="fas fa-long-arrow-alt-right" aria-hidden="true"></i> ' + value[index].new + '<br>'
            }
        ...
        function deployedLocationFormatter(row, value) {
            if ((row) && (row != undefined)) {
                ...
                return '<a href="{{ url('/') }}/locations/' + value.rtd_location.id +
                    '" data-toggle="tooltip" title="Default Location">' + value.rtd_location.name + '</a>';
            }
        ...
        function departmentNameLinkFormatter(value, row) {
            if ((row.assigned_user) && (row.assigned_user.department) && (row.assigned_user.department.name)) {
                return '<a href="{{ url('/') }}/department/' + row.assigned_user.department.id + '">' + row
        ...
        function assetCompanyObjFilterFormatter(value, row) {
            if ((row) && (row.company)) {
                return '<a href="{{ url('/') }}/hardware/?company_id=' + row.company.id + '">' + row.company.name +
                    '</a>';
            }
        }
        ...
        function employeeNumFormatter(value, row) {
            ...
            if ((row) && (row.assigned_to) && ((row.assigned_to.employee_number))) {
                return '<a href="{{ url('/') }}/users/' + row.assigned_to.id + '">' + row.assigned_to
                    .employee_number + '</a>';
            }
        }
        ...
        function orderNumberObjFilterFormatter(value, row) {
            if (value) {
                return '<a href="{{ url('/') }}/hardware/?order_number=' + row.order_number + '">' + row
                    .order_number + '</a>';
            }
        }
        ...
        function downloadImageFormatter(value, row) {
            if (value) {
                // Determine the alt text based on available data
                var altName = '';
                if (row.avatar) {
                    altName = '';
                } else if (row.name) {
                    altName = row.name;
                } else if (row.model && row.model.name) {
                    altName = row.model.name;
                }

                var thumbnailHeight = {{ $snipeSettings->thumbnail_max_h }};

                // HTML string for the image and download button
                var imageHtml = `
            <div style="display: flex; align-items: center;">
                <a href="${value}" data-toggle="lightbox" data-type="image">
                    <img src="${value}" style="max-height: ${thumbnailHeight}px; width: auto;" class="img-responsive" alt="${altName}">
                </a>
                <a href="${value}" download="${altName || 'image'}" style="margin-left: 10px;">
